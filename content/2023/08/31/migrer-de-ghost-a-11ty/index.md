...
Et voilà, vous avez tout. Enfin… vous avez vos backup de billet mais, même si *JSON* c'est bien c'est quand même pas du markdown, ce qui est la façon pour **Eleventy** d'écrire des billets. Il va donc falloir transformer tout ça.
...
Et pour ce qui est du `slug`, ça va être simple, vous l'avez déjà en fait, il est dans le *frontmatter* de chaque fichier *.md*, c'est fourni grâce à **ghost-to-md**. Vous n'avez qu'à faire en sorte que le nom de votre dossier soit raccords avec ce *slug*. Une fois fait vous pouvez virer ce `slug` du *frontmatter* il ne nous servira plus.
